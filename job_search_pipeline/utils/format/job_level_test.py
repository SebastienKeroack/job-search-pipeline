...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param("Software Engineer", "N/A"),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param("Developer", "Looking for a principal engineer", "senior"),
        pytest.param("Developer", "Open to interns and part-time applicants (internship)", "intern"),
    ],
)
def test_transform_edge_cases_and_variations(title: str, description: str, expected: str):
    assert transform(title, description) == expected
...
def test_transform_french_levels(title: str, description: str, expected: str):
    assert transform(title, description) == expected
...
def test_job_level_from_files(path: Path):
    ...
    stem = path.stem
    expected_level = "N/A" if stem.startswith("na-") else stem.lower()

    # Split file into title (first non-empty line) and description (rest)
    lines = [ln for ln in (l.strip() for l in text.splitlines()) if ln != ""]
    title = lines[0] if lines else ""
    description = "\n".join(lines[1:]) if len(lines) > 1 else ""

    assert transform(title, description) == expected_level
